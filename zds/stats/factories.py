...
class LogFactory():
    ...
    def __init__(self, content=None):
        ...
        self.time_local = "{}:{} {}".format(fake.date(pattern="%d/%b/%Y"), fake.time(pattern="%H:%M:%S"), fake.timezone())
        if content == "article":
            fake_content = Article.objects.filter(sha_public__isnull=False).order_by('?').first()
        elif content == "tutorial":
            fake_content = Tutorial.objects.filter(sha_public__isnull=False).order_by('?').first()
        elif content == "part":
            fake_content = Part.objects.filter(tutorial__sha_public__isnull=False).order_by('?').first()
        elif content == "chapter":
            fake_content = Chapter.objects.filter(part__tutorial__sha_public__isnull=False).order_by('?').first()
        else:
            fake_content = None
